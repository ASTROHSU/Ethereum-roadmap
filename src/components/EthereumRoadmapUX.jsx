...
export default function EthereumRoadmapUX() {
  ...
  return (
    ...
    >
      ...
      {/* ── Footer ── */}
      <footer
        className={`relative z-10 max-w-4xl mx-auto px-4 py-10 md:py-14 mt-8 border-t transition-colors duration-300 ${darkMode ? "border-slate-700" : "border-slate-200"}`}
      >
        {/* 區塊勢 section */}
        <div className="flex flex-col items-center gap-4 mb-8">
          <img
            src="/blocktrend-logo.png"
            alt="區塊勢 BlockTrend Logo"
        ...
        >
          <p className="leading-relaxed mb-2">
            本頁面 fork 自{" "}
            <a
              href="https://ethereum-roadmap.vercel.app/"
              target="_blank"
              rel="noopener noreferrer"
              className="font-semibold text-indigo-500 hover:text-indigo-700 transition-colors"
            >
              Ethereum Foundation Protocol Support team
            </a>{" "}
            的以太坊路線圖網站，並由區塊勢進行中文翻譯與內容延伸。
          </p>
          <a
            href="https://ethereum-roadmap.vercel.app/"
            target="_blank"
            rel="noopener noreferrer"
            className={`inline-flex items-center gap-1 text-xs hover:text-indigo-500 transition-colors ${darkMode ? "text-slate-600 hover:text-indigo-400" : "text-slate-400"}`}
          >
            <ExternalLink className="w-3 h-3" />
            前往原始網站 ethereum-roadmap.vercel.app
          </a>
        </div>
      </footer>
    </div>
